...
class TaxiData: 
    ...
    def rows(self, offset=0, limit=50, columns=None, time_of_day=None):
        ...
        return {"offset": offset, "limit":limit, "total": total, "returned": len(df), "rows": df.to_dict("records")}
    ...
    def stats(self):
        ...
        if "Time_of_Day" in self.df.columns:
            ...
            out["time_of_day_values"] = sorted(self.df["Time_of_Day"].dropna().unique().tolist())
        return out
    
    def make_X(self, Trip_Distance_km, Trip_Duration_Minutes, Time_of_Day, Passenger_Count):
        return pd.DataFrame([{
